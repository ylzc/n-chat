...
export class ChatGateway implements OnGatewayConnection, OnGatewayInit, OnGatewayDisconnect {
    ...
    async joinRoom(@MessageBody() data, @ConnectedSocket() client: Socket) {
        try {
            const space = await this.space.checkUserInSpace(data.spaceId, this.chat.getUser(client).id);
            if (space) {
                client.join(space.id);
            }
        } catch (e) {
	...
	async leaveRoom(@MessageBody() data, @ConnectedSocket() client: Socket) {
		try {
			client.leave(data.spaceId);
		} catch (e) {
